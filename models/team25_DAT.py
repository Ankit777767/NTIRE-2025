...
class PyramidAttention(nn.Module):
    def __init__(self, dim, reduction_ratio=16, num_scales=3):
        """
        ...
        Args:
            ...
            num_scales (int): Number of scales to capture multi-scale features.
        """
        super(PyramidAttention, self).__init__()
        self.num_scales = num_scales
        self.pooling_layers = nn.ModuleList([
            nn.AdaptiveAvgPool2d(output_size=(s, s)) for s in range(1, num_scales + 1)
        ])

        # Shared MLP layer to process each pooled feature
    ...
    def forward(self, x):
        """
        Forward pass for Pyramid Attention.
        ...
        for pool in self.pooling_layers:
            pooled = pool(x)  # Adaptive pooling to different scales
            upsampled = F.interpolate(pooled, size=(H, W), mode='bilinear', align_corners=False)
            attention_maps.append(upsampled)

        # Aggregate multi-scale features
        attention = torch.mean(torch.stack(attention_maps, dim=0), dim=0)  # Average over scales
        attention = self.mlp(attention)  # Apply shared MLP
        return x * attention  # Scale input by learned attention weights
...
class DATB(nn.Module):
    def __init__(self, dim, num_heads, reso=64, split_size=[2,4], shift_size=[1,2], expansion_factor=4.,
                 ...
                 norm_layer=nn.LayerNorm, rg_idx=0, b_idx=0, total_blocks=6):
        """
        DAT Block with Hybrid Attention (Spatial, Channel, and Pyramid).
        
        Args:
            dim (int): Number of channels.
            num_heads (int): Attention heads.
            reso (int): Resolution of input feature map.
            split_size (list): Split sizes for Adaptive Spatial Attention.
            shift_size (list): Shift sizes for Adaptive Spatial Attention.
            expansion_factor (float): Expansion factor for Feedforward Network.
            rg_idx (int): Residual Group index.
            b_idx (int): Block index within the group.
            total_blocks (int): Total number of blocks in the network.
        """
        super().__init__()
        ...
        if b_idx < total_blocks // 3:  # Early Layers → Spatial Attention
            self.attn = Adaptive_Spatial_Attention(
                dim, num_heads=num_heads, reso=reso, split_size=split_size, shift_size=shift_size, 
                qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop, rg_idx=rg_idx, b_idx=b_idx
            )
        elif b_idx < 2 * total_blocks // 3:  # Middle Layers → Channel Attention
            self.attn = Adaptive_Channel_Attention(
                dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop
            )
        else:  # Deeper Layers → Pyramid Attention
            self.attn = PyramidAttention(dim)
    ...
    def forward(self, x, x_size):
        """
        Forward pass for DATB with Hybrid Attention.
        
        Args:
            x (torch.Tensor): Input tensor of shape (B, H*W, C).
            x_size (tuple): Spatial size (H, W).
...
class ResidualGroup(nn.Module):
    def __init__(self, dim, reso, num_heads, split_size=[2,4], expansion_factor=4.,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, depth=2, use_chk=False, 
                 resi_connection='1conv', rg_idx=0):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            DATB(
                dim=dim, num_heads=num_heads, reso=reso, split_size=split_size, shift_size=[split_size[0]//2, split_size[1]//2],
                expansion_factor=expansion_factor, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
                drop_path=drop_paths[i], act_layer=act_layer, norm_layer=norm_layer, rg_idx=rg_idx, b_idx=i, 
                total_blocks=depth  # 👈 Added this for Hybrid Attention
...
class DAT(nn.Module):
    def __init__(self, img_size=64, in_chans=3, embed_dim=180, split_size=[2,4], depth=[2,2,2,2],
                 num_heads=[2,2,2,2], expansion_factor=4., qkv_bias=True, qk_scale=None, drop_rate=0., 
                 attn_drop_rate=0., drop_path_rate=0.1, act_layer=nn.GELU, norm_layer=nn.LayerNorm, 
                 use_chk=False, upscale=2, img_range=1., resi_connection='1conv', upsampler='pixelshuffle', **kwargs):
        super().__init__()
        ...
        for i in range(self.num_layers):
            layer = ResidualGroup(
                dim=embed_dim, num_heads=heads[i], reso=img_size, split_size=split_size, expansion_factor=expansion_factor,
                qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, 
                drop_paths=dpr[sum(depth[:i]):sum(depth[:i + 1])], act_layer=act_layer, norm_layer=norm_layer, 
                depth=depth[i], use_chk=use_chk, resi_connection=resi_connection, rg_idx=i
            )  # 👈 Pass total_blocks=depth[i]
